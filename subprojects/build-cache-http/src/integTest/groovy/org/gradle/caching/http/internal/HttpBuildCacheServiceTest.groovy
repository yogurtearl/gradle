...
import static org.gradle.caching.BuildCacheService.StoreOutcome.NOT_STORED
import static org.gradle.caching.BuildCacheService.StoreOutcome.STORED
...
class HttpBuildCacheServiceTest extends Specification {
    ...
    HttpAsyncClientHelper.Factory httpClientHelperFactory = HttpAsyncClientHelper.Factory.createFactory(new DocumentationRegistry())
    ...
    def "can cache artifact"() {
        ...
        when:
        def outcome = cache.maybeStore(key, writer(content))
    ...
    def "can load artifact from cache"() {
        ...
        cache.load(key, new LoadTarget(file))
    ...
    def "loading from cache does follow redirects"() {
        ...
        def reader = new LoadTarget(file)
        cache.load(key, reader)
    ...
    def "reports cache miss on 404"() {
        ...
        when:
        def file = tempDir.createFile("entry")
        def fromCache = cache.load(key, new LoadTarget(file))
    ...
    def "store reports error on http code #httpCode"(int httpCode) {
        ...
        when:
        cache.maybeStore(key, writer("".bytes))
    ...
    def "can use expect continue to avoid sending body"() {
        ...
        when:
        def result = cache.maybeStore(key, writer("abc".bytes))
    ...
    def "does not throw error when server responds with too large without reading body"() {
        ...
        def result = cache.maybeStore(key, new StoreTarget(file))
    ...
    def "does transmit body when using expect continue that continues"() {
        ...
        when:
        def outcome = cache.maybeStore(key, writer)
    ...
    def "sends X-Gradle-Version and Content-Type headers on GET"() {
        ...
        expect:
        cache.load(key, new LoadTarget(tempDir.createFile("file")))
    }
    ...
    def "sends X-Gradle-Version and Content-Type headers on PUT"() {
        ...
        expect:
        cache.maybeStore(key, writer("".bytes)) == STORED
    }
    ...
    private StoreTarget writer(byte[] content) {
        def file = tempDir.createFile(UUID.randomUUID().toString())
        file.bytes = content
        new StoreTarget(file)
    }

...
class HttpBuildCacheServiceErrorHandlingIntegrationTest extends HttpBuildCacheFixture {
    ...
    def "build does not fail if connection repeatedly drops during store"() {
        ...
        settingsFile << withHttpBuildCacheServer()
        String errorPattern = /(Broken pipe|Connection reset|Connection reset by peer|Software caused connection abort: socket write error|An established connection was aborted by the software in your host machine|127.0.0.1:.+ failed to respond)/
